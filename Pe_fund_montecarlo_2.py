...
with col_A:
    ...
    with st.container(border=True):
        ...
        inv_std_pct = st.slider(
            ...
            format="%d%%",
            help=(
                "Each year's investment amount is drawn from Normal(mean, mean × this%). "
                "Mean = Fund Size × Investable% ÷ 5. "
        ...
        st.caption(
            ...
            f"±1 std dev range ≈ €{_lo_ref:.0f}M – €{_hi_ref:.0f}M"
        )

        # Mini preview: show ±1σ band on a normal curve
        ...
        st.markdown(
            f'<div class="prev-label">Normal(mean, mean × {inv_std_pct*100:.0f}%), clipped at 0</div>',
            unsafe_allow_html=True
        )
